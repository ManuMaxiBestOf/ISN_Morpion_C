...
// ##########
    ...
    //Afin d'aovir des instructions non bloquantes, il faut créer un set de sockets.
    //On regardera alors si un des sockets dans le set est actif avec SDLNet_CheckSocket.
    //Si un socket est actif, cette fonction le "marque" comme actif.
    ...
    int connexion_client_serveur( char * adresse_ip, unsigned int port)
    {
        ...
        {
            set = SDLNet_AllocSocketSet(1);//1 est le nombre de sockets dans le set.
            SDLNet_TCP_AddSocket(set, client);  //Ajoute le socket desormais actif au set.
            return 1;
        }
    ...
    int serveur_accepte_client()
    {
        ...
        {
            set = SDLNet_AllocSocketSet(16);//1 est le nombre de sockets dans le set.
            SDLNet_TCP_AddSocket(set, client);  //Ajoute le socket desormais actif au set.
            return 1;
        }
    ...
    // 9.5 Lecture d'un message. Renvoie le text envoyé.
    // Exemple de déclaration d'une variable qui contiendra la chaine de caractère recue:
    // char* message_recu;
    char* lire_message()
    {
    ...
    char* lire_message_non_bloquant()
    {
        char* reponse=malloc(_TAILLE_RESEAU_TRANSFERT_*sizeof(char));
        if(SDLNet_CheckSockets(set, 0)) //vérifie si l'un des sockets du set est actif, en attendant 0 seconde
        {
            if(SDLNet_SocketReady(client)) //normalement inutile, vu qu'il n'y a qu'un socket
            {
                SDLNet_TCP_Recv(client, reponse, _TAILLE_RESEAU_TRANSFERT_ +1);
                return reponse;
            }
    ...
    //  - soit sous format d'une chaine de caractère (exemple: "Coucou")
    //  - soit sous forme d'un tableau de caractères comportant_TAILLE_RESEAU_TRASNFERT_ caracteres.
    // Exemple de déclaration d'une variable qui contiendra la chaine de caractère à envoyer:
